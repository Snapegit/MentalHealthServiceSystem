...
<template>
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<div class="btn_view">
					...
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('yonghu','年龄统计')">年龄统计</el-button>
					<el-button type="warning" @click="echartClick2" v-if="btnAuth('yonghu','性别统计')">性别统计</el-button>
				</div>
			</div>
			<br>
			<el-table
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'年龄统计'" width="70%">
			<el-tabs v-model="echartActive" class="demo-tabs" @tab-change="echartTabClick" type="card">
			    <el-tab-pane label="年龄统计" name="1"></el-tab-pane>
			    <el-tab-pane label="性别统计" name="2"></el-tab-pane>
			</el-tabs>
			<div v-if="echartActive==1" id="nianlingEchart1" style="width:100%;height:600px;"></div>

			<div v-if="echartActive==2" id="xingbieEchart2" style="width:100%;height:600px;"></div>
...
<script setup>
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var nianlingEchart1 = echarts.init(document.getElementById("nianlingEchart1"),'macarons');
			context?.$http({
				...
				// 使用刚指定的配置项和数据显示图表。
				nianlingEchart1.setOption(option);
				  //根据窗口的大小变动图表
				window.onresize = function() {
				    nianlingEchart1.resize();
				};
			})
		})
	}
	...
	const echartClick2 = ()=>{
		...
		echartVisible.value = true
		nextTick(()=>{
			var xingbieEchart2 = echarts.init(document.getElementById("xingbieEchart2"),'macarons');
			context?.$http({
				...
				// 使用刚指定的配置项和数据显示图表。
				xingbieEchart2.setOption(option);
				  //根据窗口的大小变动图表
				window.onresize = function() {
				    xingbieEchart2.resize();
				};
			})
		})
	}
