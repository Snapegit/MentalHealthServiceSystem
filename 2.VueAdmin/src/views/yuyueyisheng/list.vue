	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('yuyueyisheng','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('yuyueyisheng','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('yuyueyisheng','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('yuyueyisheng','删除')">删除</el-button>
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('yuyueyisheng','预约次数')">预约次数</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column prop="ispay" label="支付状态" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						...
						<el-button v-if="scope.row.ispay!='已支付' && btnAuth('yuyueyisheng','支付')" type="text" size="small" @click.stop="payClick(scope.row)">支付</el-button>
					</template>
				</el-table-column>
				<el-table-column
					 :resizable='true' 
				...
				<el-table-column label="审核" v-if="btnAuth('yuyueyisheng','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('yuyueyisheng','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('yuyueyisheng','取消预约')" type="success" @click="quxiaoyuyueCrossAddOrUpdateHandler(scope.row,'cross','是','','')">取消预约</el-button>
						<el-button v-if="btnAuth('yuyueyisheng','就诊')" type="success" @click="yishengjiuzhenCrossAddOrUpdateHandler(scope.row,'cross','否','','')">就诊</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'预约次数'" width="70%">
...
<script setup>
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var keyuerenshuEchart1 = echarts.init(document.getElementById("keyuerenshuEchart1"),'macarons');
			context?.$http({
				url: `yuyueyisheng/value/yonghuzhanghao/keyuerenshu`,
				method: 'get'
			}).then(res=>{
    ...
    const quxiaoyuyueCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			quxiaoyuyueFormModelRef.value.init(row.id,'cross','取消预约',row,'yuyueyisheng',statusColumnName,tips,statusColumnValue)
		})
    }
    ...
    const yishengjiuzhenCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			yishengjiuzhenFormModelRef.value.init(row.id,'cross','就诊',row,'yuyueyisheng',statusColumnName,tips,statusColumnValue)
		})
    }
	//查询审核状态列表
