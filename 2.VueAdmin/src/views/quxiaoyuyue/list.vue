	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('quxiaoyuyue','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('quxiaoyuyue','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('quxiaoyuyue','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('quxiaoyuyue','删除')">删除</el-button>
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('quxiaoyuyue','取消次数')">取消次数</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="审核" v-if="btnAuth('quxiaoyuyue','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('quxiaoyuyue','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'取消次数'" width="70%">
...
<script setup>
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			...
			context?.$http({
				url: `quxiaoyuyue/value/yonghuzhanghao/keyuerenshu`,
				method: 'get'
			}).then(res=>{
