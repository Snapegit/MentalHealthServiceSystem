	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('zuozhenyisheng','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('zuozhenyisheng','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('zuozhenyisheng','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('zuozhenyisheng','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				ref="table"
				v-if="btnAuth('zuozhenyisheng','查看')"
				:data="list"
				...
				<el-table-column
					...
					label="就诊地点">
					<template #default="scope">
						{{scope.row.jiuzhendidian}}
					</template>
				</el-table-column>
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('zuozhenyisheng','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('zuozhenyisheng','预约')" type="success" @click="yuyueyishengCrossAddOrUpdateHandler(scope.row,'cross','','','')">预约</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<yuyueyishengFormModel ref="yuyueyishengFormModelRef" @formModelChange="formModelChange"></yuyueyishengFormModel>
	</div>
</template>
<script setup>
	...
	import yuyueyishengFormModel from '@/views/yuyueyisheng/formModel'
	const yuyueyishengFormModelRef = ref(null)
    const yuyueyishengCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			yuyueyishengFormModelRef.value.init(row.id,'cross','预约',row,'zuozhenyisheng',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
