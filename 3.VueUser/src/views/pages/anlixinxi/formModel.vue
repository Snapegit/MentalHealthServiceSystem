	...
	<div class="app-contain" :style='{"padding":"20px 20px","margin":"20px auto 40px","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"1200px","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">案例类型</div>
					<div  class="info_text" >{{detail.anlileixing}}</div>
				</div>
				<div class="info_item">
				...
				<div class="btn_view">
					<el-button v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnFrontAuth('anlixinxi','支付')" class="approval_btn" @click="payClick">支付</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('anlixinxi','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('anlixinxi','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
		<div class="video_view">
			<video :src="$config.url + detail.anlishipin" controls autoplay></video>
		</div>
		<el-tabs type="border-card" v-model="activeName" class="tabs_view">
			<el-tab-pane label="案例详情" name="first">
				<div v-html="detail.anlixiangqing"></div>
			</el-tab-pane>
		</el-tabs>
	</div>
</template>
<script setup>
	...
	const getDetail = () => {
		context?.$http({
			...
			title.value = res.data.data.anlimingcheng
