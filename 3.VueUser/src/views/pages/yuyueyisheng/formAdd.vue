	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0","margin":"20px auto 60px","borderRadius":"20px","background":"none","width":"1200px","position":"relative","height":"100%"}'>
		...
		<el-form ref="formRef" :model="form" class="add_form" label-width="120px" :rules="rules">
			<el-row>
				...
				<el-col :span="12">
					<el-form-item label="预约数量" prop="keyuerenshu">
						<el-input class="list_inp" v-model.number="form.keyuerenshu" placeholder="预约数量"
				...
				<el-col :span="12">
					<el-form-item label="住址" prop="zhuzhi">
						<el-input class="list_inp" v-model="form.zhuzhi" placeholder="住址"
							 type="text" 							:readonly="!isAdd||disabledForm.zhuzhi?true:false" />
					</el-form-item>
				</el-col>
				...
				<el-col :span="12">
					<el-form-item label="就诊状态" prop="jiuzhenzhuangtai">
						<el-select
							class="list_sel"
							:disabled="!isAdd||disabledForm.jiuzhenzhuangtai?true:false"
							v-model="form.jiuzhenzhuangtai" 
							placeholder="请选择就诊状态"
							style="width:100%;"
							>
							<el-option v-for="(item,index) in jiuzhenzhuangtaiLists" :label="item"
								:value="item"
								>
							</el-option>
						</el-select>
...
<script setup>
	...
	//就诊状态列表
	const jiuzhenzhuangtaiLists = ref([])
	//methods
	...
	const init = (formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null) => {
		...
		context?.$http({
			...
			if(json.hasOwnProperty('zhuzhi') && context?.$toolUtil.storageGet("frontRole")!="管理员"){
				form.value.zhuzhi = json.zhuzhi
				disabledForm.value.zhuzhi = true;
			}
		})
		jiuzhenzhuangtaiLists.value = "初诊,复诊".split(',')
	}
	//初始化
	//取消
	const backClick = () => {
	...
	const save=()=>{
		...
		if(objcross!='') {
			...
			if(objcross.keyuerenshu<0){
				context?.$toolUtil.message('预约数量不足','error')
				return false
			}
		}
		let crossUserId = ''
